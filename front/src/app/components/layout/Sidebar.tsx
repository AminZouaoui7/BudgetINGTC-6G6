...
const menuItems = [
  ...
  { icon: Users, label: 'Administration', path: '/admin/users', adminOnly: true }
];
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto">
        {menuItems
          .filter((item) => !(item.adminOnly && !user?.role?.includes('admin')))
          .map((item) => {
      ...
      <div className="p-3 border-t border-[#bce3fb]/30">
        <div className="group relative">
          <div className={`flex items-center gap-3 p-2 rounded-lg hover:bg-[#f0f8ff] cursor-pointer transition-colors ${isCollapsed ? 'justify-center' : ''}`}>
            ...
            {!isCollapsed && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-900 truncate">{user?.fullName || 'Utilisateur'}</p>
                <p className="text-xs text-gray-500 truncate capitalize">{user?.role || 'Chargement...'}</p>
              </div>
            )}
          </div>
          ...
          <button
            ...
            className={`mt-2 flex items-center gap-3 w-full p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors ${isCollapsed ? 'justify-center' : ''}`}
